...
const WORD_TARGET=8;
const defaultState={xp:0,xpVersion:XP_VERSION,answered:0,correct:0,wordMastery:{},wordCorrectCounts:{},grammarVersion:GRAMMAR_VERSION,grammarProgress:{},streak:1,sound:true,activity:[0,0,0,0,0,0,0]};
...
function renderGrammar(level='all'){
 ...
 content.innerHTML=`<section class="page"><span class="eyebrow">Grammar gym</span><h1>Repeat until it sticks.</h1><p class="intro">Topics contain 20–48 exercises, depending on their scope. Complete one full set and reach the displayed target to pass (at least 87.5% correct, rounded up). Repeating carries your score forward; each new answer replaces the oldest. Your place is saved after every answer.</p><div class="filters">${['all','A1','A2'].map(x=>`<button class="filter ${x===level?'active':''}" data-level="${x}">${x==='all'?'All rules':x}</button>`).join('')}</div><div class="rule-list">${list.map(r=>{
...
function pendingWords(pool){return pool.filter(word=>wordCorrect(word)<WORD_TARGET)}
...
function answer(value,button){if(session.locked)return;session.locked=true;let ok=matchesAnswer(value);state.answered++;if(ok){state.correct++;session.roundCorrect++;state.xp+=XP_REWARDS[session.type]};state.activity[new Date().getDay()?new Date().getDay()-1:6]++;if(session.type==='words'){let id=session.current.word.id;state.wordCorrectCounts??={};if(ok)state.wordCorrectCounts[id]=Math.min(WORD_TARGET,wordCorrect(session.current.word)+1);state.wordMastery[id]=Math.round(wordCorrect(session.current.word)/WORD_TARGET*100)}else{const id=session.rule.id;const wasPassed=ruleStatus(id).passed;state.grammarProgress??={};state.grammarProgress[id]=recordGrammarAnswer(state.grammarProgress[id],ok,id);session.answered=(session.answered||0)+1;session.justPassed=!wasPassed&&ruleStatus(id).passed;$('#grammarScore').innerHTML=grammarScore(id)}if(button){button.classList.add(ok?'correct':'wrong');document.querySelectorAll('.choice').forEach(b=>{if(matchesAnswer(b.dataset.answer))b.classList.add('correct');b.disabled=true})}let f=$('#feedback');f.className='feedback '+(ok?'good':'bad');f.innerHTML=`<div><strong>${ok?'Richtig!':'Not quite'}</strong><br>${answerFeedback(ok)}</div><button class="primary" id="next">Continue</button>`;save();$('#next').onclick=()=>{session.index++;session.locked=false;if(session.type==='words')renderWordQuestion();else renderRuleQuestion()}}
function finishSession(){
 if(session.type==='grammar')return finishGrammar();
 const pct=Math.round(session.roundCorrect/session.items.length*100);
 content.innerHTML=`<div class="practice-wrap"><section class="practice"><span class="eyebrow">Round complete</span><h1>${pct>=80?'Sehr gut!':'Keep building.'}</h1><p class="intro">You answered ${session.roundCorrect} of ${session.items.length} correctly.</p><div class="stats"><div class="stat"><strong>${pct}%</strong><small>this round</small></div><div class="stat"><strong>+${session.roundCorrect*XP_REWARDS.words}</strong><small>XP earned</small></div><div class="stat"><strong>${session.items.length}</strong><small>words reviewed</small></div></div><div class="type-row"><button class="primary" id="again">Practise again</button><button class="secondary" id="done">Done</button></div></section></div>`;
  ...
  try{
    ...
    if(account){
      ...
      try{
        ...
        if(stored&&(stored.xpVersion!==XP_VERSION||stored.grammarVersion!==GRAMMAR_VERSION))save();
