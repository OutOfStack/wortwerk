...
const WORD_TARGET=8;
const defaultState={xp:0,xpVersion:XP_VERSION,answered:0,correct:0,wordMastery:{},wordCorrectCounts:{},ruleMastery:{},streak:1,sound:true,activity:[0,0,0,0,0,0,0]};
...
function updateChrome(){
 ...
 const mastered=Object.values(state.ruleMastery).filter(x=>x>=85).length;
 ...
 $('#totalMastery').textContent=mastered+' grammar skills mastered';
...
function renderProgress(){const best=bestRule();const acc=state.answered?Math.round(state.correct/state.answered*100):0;const mastered=Object.values(state.ruleMastery).filter(x=>x>=85).length;const max=Math.max(...state.activity,1);content.innerHTML=`<section class="page"><span class="eyebrow">Learning record</span><h1>Your progress.</h1>${rankOverview()}<div class="stats"><div class="stat"><strong>${mastered}/${RULES.length}</strong><small>grammar rules mastered</small></div><div class="stat"><strong>${acc}%</strong><small>all-time accuracy</small></div><div class="stat"><strong>${state.xp}</strong><small>experience points</small></div></div>${wordProgress()}<div class="progress-grid"><div class="chart"><h3>Last 7 practice days</h3><div class="bars">${state.activity.map((v,i)=>`<span class="daybar" style="height:${Math.max(5,v/max*100)}%"><small>${['M','T','W','T','F','S','S'][i]}</small></span>`).join('')}</div></div><div class="summary-card"><span class="eyebrow">Strongest rule</span><h2>${best?.title||'Start practising'}</h2><p class="intro">${best?`${state.ruleMastery[best.id]}% rolling mastery`:'Your first completed round will appear here.'}</p><button class="secondary" id="resetBtn">Reset progress</button></div></div></section>`;$('#resetBtn').onclick=()=>{if(confirm(account?'Reset all saved progress for this account?':'Reset all guest progress on this device?')){state=structuredClone(defaultState);save();renderProgress()}}}
  ...
  try{
    ...
    if(account){
      ...
      try{
        ...
        if(stored&&stored.xpVersion!==XP_VERSION)save();
